...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0, onActiveItemChange }) => {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      ...
      setActiveItem(item);
      if (hasInteractedRef.current) onActiveItemChange?.(item);
    };
    ...
    if (canvas) {
      sketch = new InfiniteGridMenu(
        ...
        handleActiveItem,
        (moving) => {
          if (moving) hasInteractedRef.current = true;
          setIsMoving(moving);
        },
        sk => sk.run(),
        scale
