...
- Pages are server components with `export const revalidate = 86400`. The window is a safety net, not the publishing mechanism — tag invalidation is (see below). Nothing on the public site reads `searchParams`: doing so marks the route dynamic and makes every visit boot Payload and open a Mongo connection. `/portfolio` used to, for `?cliente=`; that filter now lives at `/portfolio/cliente/[slug]`, prerendered once per client from `getClientSlugs()`, with a 301 from the old query in `next.config.ts`. Keep it that way — a single `searchParams` read is the difference between a page that costs nothing and one that costs a function per visitor.
- Every reader in `lib/content.ts` goes through `cachedRead()` (`lib/cache.ts`), which stacks React `cache()` (per-request dedupe) on `unstable_cache` (across requests) and carries a tag per collection/global. Collections and globals invalidate their tag on save via `lib/revalidate.ts`, so an edit shows up on the next request and the expiry window is only a safety net (10 minutes). Add a reader → give it a key and the tags it depends on; add a collection → give it `revalidatesCollection(...)`. `media` invalidates everything that can show an image, because swapping a file changes the resolved `src` without touching the document that references it.
