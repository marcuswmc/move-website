...
- Pages are server components with `export const revalidate = 600`. `/portfolio` renders per request to apply `searchParams` in its initial HTML; its project data, page settings and shared site settings come from the same cached readers.
